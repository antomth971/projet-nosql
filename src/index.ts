...
//neo4j
const neo = neo4j.driver(process.env.NEO4J_URI!, neo4j.auth.basic(process.env.NEO4J_USER!, process.env.NEO4J_PASSWORD!));
...
app.get("/offers", async (c) => {
  ...
  try {
    ...
    // cache hit
    const cached = await redis.get(commandOptions({ returnBuffers: true }), key);
    if (cached) {
      return c.json(JSON.parse(gunzipSync(cached).toString()));
    }

    // Mongo query
    const docs = await offers.find({ from, to }).sort({ price: 1 }).limit(limit).toArray();
...
app.get("/offers/:id", async (c) => {
  ...
  const relatedDocs = await offers
    .find({ from: offer.from, to: { $in: nearCodes }, date: offer.date }, { projection: { _id: 1 } })
    .limit(3)
    .toArray();
...
// Servir les fichiers statiques du dossier public (fallback pour toutes les routes non-API)
